...
namespace ClassificadorDoc.Controllers
{
    ...
    public class ClassificadorController : ControllerBase
    {
        ...
        [HttpPost("classificar-pdf-visual")]
        [RequestSizeLimit(10_000_000)] // 10MB limit para PDF individual
        public async Task<ActionResult<DocumentoClassificacao>> ClassificarPdfVisual(IFormFile arquivo)
        {
            try
            {
                if (arquivo == null || arquivo.Length == 0)
                ...
                {
                    return BadRequest("O arquivo deve ter extensão .pdf");
                }

                _logger.LogInformation("Processando PDF visual: {NomeArquivo}", arquivo.FileName);
                ...
                _logger.LogInformation("PDF analisado visualmente como: {Tipo} com confiança {Confianca}",
                    classificacao.TipoDocumento, classificacao.ConfiancaClassificacao);

                return Ok(classificacao);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erro ao processar PDF visual");
                return StatusCode(500, $"Erro interno: {ex.Message}");
            }
        }

        [HttpPost("classificar-pdf-individual")]
        [RequestSizeLimit(10_000_000)] // 10MB limit para PDF individual
        public async Task<ActionResult<DocumentoClassificacao>> ClassificarPdfIndividual(IFormFile arquivo)
        {
            try
            {
                if (arquivo == null || arquivo.Length == 0)
                ...
                _logger.LogInformation("Processando PDF individual: {NomeArquivo}", arquivo.FileName);

                using var pdfStream = arquivo.OpenReadStream();
                var texto = await _pdfExtractor.ExtrairTextoAsync(pdfStream);

                if (string.IsNullOrWhiteSpace(texto))
                {
                    return BadRequest("Não foi possível extrair texto do PDF ou o arquivo está vazio.");
                }

                var classificacao = await _classificador.ClassificarDocumentoAsync(arquivo.FileName, texto);

                _logger.LogInformation("PDF classificado como: {Tipo} com confiança {Confianca}",
                    classificacao.TipoDocumento, classificacao.ConfiancaClassificacao);
